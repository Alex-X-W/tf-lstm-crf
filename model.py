...
class Model(object):
    ...
    """
    def __init__(self, config, parameters=None, models_path=None, model_path=None):
        """
        ...
        if model_path is None:
            ...
            with open(os.path.join(model_path, 'params'), 'w') as f:
                f.write(self.name)
    ...
    def build(self,
        ...
        word_dim = config['model_params']['embed_params']['f1']['dim']
        char_dim = config['model_params']['embed_params']['char']['dim']
        input_dim += (word_dim + char_dim)
        with open(config['model_params']['embed_params']['f1']['path'], 'rb') as file_r:
            pretrained_embed = pickle.load(file_r)
        #
        # Word inputs
        #
        with tf.device("/cpu:0"):
            word_layer = EmbeddingLayer(n_words, word_dim, name='word_layer', initializer=pretrained_embed)
        ...
        # Chars inputs
        #
        char_layer = EmbeddingLayer(n_chars, char_dim, name='char_layer', initializer=None)
