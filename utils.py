...
def create_input_batch(sentences, parameters, n_tags=0, add_label=False, singletons=None):
    ...
    for k in range(batch_size):
        ...
        if add_label:
            ...
            if max_length > tag_len:
                for i in range(max_length - tag_len):
                    tag_array.append(0)
    ...
    if parameters['char_dim']:
        ...
        count = 0
        char_pos_array = []
        for i in range(len(chars_batch)):
            chars = chars_batch[i]
...
def evaluate(sess, f_eval, model, parameters, parsed_sentences, n_tags=0):
    ...
    token_accuracy = old_div(np.sum(token_accus_all), (token_count + 0.000001))
