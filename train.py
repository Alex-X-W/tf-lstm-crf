...
parameters['word_dim'] = config['model_params']['embed_params']['f1']['dim']
parameters['word_lstm_dim'] = config['model_params']['embed_params']['f1']['dim']
...
# Initialize model
model = Model(config, parameters=parameters, models_path=models_path)
...
# Load sentences
train_sentences = loader.load_sentences(config['data_params']['path_train'], lower, zeros)
dev_sentences = loader.load_sentences(config['data_params']['path_dev'], lower, zeros)
test_sentences = loader.load_sentences(config['data_params']['path_test'], lower, zeros)

# Create a dictionary / mapping of words
word_to_id, id_to_word = word_mapping(config)
...
if opts.gpu_no < 0:
    with tf.device("/cpu:0"):
        cost, tags_scores, train_op = model.build(config, **parameters)
else:
    ...
    for epoch in range(n_epochs):
        ...
        while train_data_count <= len(permutation_index):
            ...
            if train_data_count % freq_eval == 0 and train_data_count > 0:
                ...
                token_accus_freq = old_div(np.sum(epoch_accus), token_count)
        ...
        assert token_count != 0.0
        token_accus_epoch = old_div(np.sum(epoch_accus), token_count)
